...
if (in_array($action, ['add', 'edit'])) {
    ...
    if ($action === 'edit' && $id) {
        ...
        // See manage.php's identical comment — warn_hours is always 0 for
        // a parts-mode rule now, so hours_to_warn_fields()'s own
        // warn_enabled derivation would be wrong for one; derive it from
        // the gap instead when that's the active mode.
